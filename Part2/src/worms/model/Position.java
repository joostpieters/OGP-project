...
public class Position {
	 ...
	 */
	@Raw
	public Position(final double x, final double y) throws IllegalCoordinateException {
	 ...
	 */
	public static boolean isValidCoordinate(final double cor) {
	 ...
	 */
	@Raw
	public void setX(final double x) throws IllegalCoordinateException{
		if (!isValidCoordinate(x))
			throw new IllegalCoordinateException(x);
		this.x = x;
	}
	 ...
	 */
	@Raw
	public void setY(final double y) throws IllegalCoordinateException{
		if (!isValidCoordinate(y))
			throw new IllegalCoordinateException(y);
		this.y = y;
	}
	 ...
	 */
	public Position getPixelPosition(final double widthScale, final double heightScale){
		return new Position(Math.round(getX() / widthScale),
							Math.round(getY() / heightScale));
	}
	 ...
	 *			|				getY() * heightScale)
	 */
	public Position getMeterPosition(final double widthScale, final double heightScale){
	 ...
	 */
	public int toXPixel(final double scale){
		return (int) Math.round(getX() / scale);
	}
	 ...
	 */
	public int toYPixel(final double scale){
		return (int) Math.round(getY() / scale);
	}
	 ...
	 * 			| result == getX() * scale
	 */
	public double toXMeters(final double scale){
		return getX() * scale;
	}
	 ...
	 * 			| result == getY() * scale
	 */
	public double toYMeters(final double scale){
		return getY() * scale;
	}
	 ...
	 */
	public Position[] getCircularPositions(final double radius){
	 ...
	 * @return	The positions around this position at a distance equal to the given radius.
	 */
	public Position[] getCircularPositions(final double radius, final double minAngle, final double maxAngle, final double intervalAngle){
		final int amountOfPositions = (int) ((maxAngle - minAngle) / intervalAngle);
		...
		for (int counter = 0; counter < positions.length; counter++){
			tmpPos = new Position
					(objX + Math.cos(currAngle) * radius,
					 objY + Math.sin(currAngle) * radius);
	 ...
	 */
	public double getDistanceTo(final Position end) {
	 ...
	 *			|				/ (getX() - end.getX()))
	 */
	public double getSlope(final Position end){
		if (getX() == end.getX())
			return 0.0;
