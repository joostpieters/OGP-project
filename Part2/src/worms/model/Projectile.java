...
public class Projectile extends BallisticObject {
	 ...
	 */
	@Raw
	public Projectile(final World world, final Worm owner, final int yield, final Weapon weapon) 
	 ...
	 * 			| result == getUpdatedVector(this.owner)
	 */
	private Vector getUpdatedVector(){
		return getUpdatedVector(this.owner);
	}
	 ...
	 */
	public static Vector getUpdatedVector(final Worm owner){
		return new Vector(
				new Position(
				owner.getVector().getPosition().getX() + 
					Math.cos(owner.getVector().getDirection()) * owner.getRadius(),
	...
	public boolean ownerCanShoot() {
		return this.owner.canShoot(this);
	}
	 ...
	 * 			| !canJump()
	 */
	public void jump(final double timeStep) {
		if (!canJump())
	...
	public boolean isJumpFinished(final Position currPos){ 
		return !getWorld().liesWithinWorld(currPos, getRadius(), true) ||
					getWorld().isImpassableSpot(currPos, getRadius()) || !isActive() ||
						getWorld().hasOverlappingWorm(this, currPos);
	}
	 ...
	 */
	public static boolean isValidYield(final int yield){
