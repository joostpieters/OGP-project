...
public abstract class BallisticObject extends Entity {
	...
	@Model
	protected BallisticObject(World world, Vector vector){
		this.isActive = true;
		this.vector = vector;
		this.world = world;
	}
	 ...
	 */
	protected boolean overlapsWith(BallisticObject o, 
			Position currPos) {
	...
	/**
	 * Calculates the time that a jump of this ballistic object in its current
	 * 	direction would take.
	 * 
	 * @param	timeStep
	 * 			The time steps to perform the calculations with.
	 * @return	The time a jump of this ballistic object would take.
	 * @throws	IllegalJumpException(this)
	 * 			The ballistic object can not perform a jump.
	 * 			| !canJump()
	 * @throws IllegalJumpException(this)
	 * 			The ballistic object can not perform a jump.
	 * 			| !(tmpPos.getDistanceTo(currPos) < getRadius() && this instance of Worm)
	 */
	@Raw
	public double getJumpTime(double timeStep) throws IllegalJumpException {
		if (!canJump())
	 ...
	 */
	public void performJump(double timeStep){
	 ...
	 */
	public abstract boolean isJumpFinished(Position currPos);
	 ...
	 * 			calculated with.
	 */
	public abstract double getInitialJumpVelocity(double force);
